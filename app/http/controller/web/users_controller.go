...
func (u *Users) Register(context *gin.Context) {
	...
	// 注意：在获取表单参数验证器中的数字键（字段）,请统一使用 GetFloat64(),其它获取数字键（字段）的函数无效，例如：GetInt()、GetInt64()等
...
func (u *Users) Login(context *gin.Context) {
	...
	userModelFact := model.CreateAdminUserModel("")
	userModel := userModelFact.Login(userName, pass)

	if userModel != nil {
		userTokenFactory := userstoken.CreateUserFactory()
		role := "u"
		if userToken, err := userTokenFactory.GenerateToken(userModel.ID, userModel.UserName, userModel.Phone, role, 0, variable.ConfigYml.GetInt64("Token.JwtTokenCreatedExpireAt")); err == nil {
...
func (u *Users) Show(context *gin.Context) {
	...
	counts, showlist := model.CreateAdminUserModel("").Show(userName, int(limitStart), int(limit))
...
func (u *Users) Destroy(context *gin.Context) {
	//表单参数验证中的int、int16、int32 、int64、float32、float64等数字键（字段），请统一使用 GetFloat64() 获取，其他函数无效
	userId := context.GetFloat64(consts.ValidatorPrefix + "id")
	if model.CreateAdminUserModel("").Destroy(int(userId)) {
		response.Success(context, consts.CurdStatusOkMsg, "")
	} else {
