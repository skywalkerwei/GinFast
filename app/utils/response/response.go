...
import (
	...
	"ginfast/app/utils/validator_translation"
...
// ErrorTokenNull token 基本的格式错误
func ErrorTokenNull(c *gin.Context) {
	//ReturnJson(c, http.StatusBadRequest, http.StatusBadRequest, my_errors.ErrorsTokenBaseInfo, "")
	ReturnJson(c, http.StatusOK, consts.JwtTokenInvalid, my_errors.ErrorTokenNullInfo, "")
	//终止可能已经被加载的其他回调函数的执行
	c.Abort()
}
...
func ErrorTokenBaseInfo(c *gin.Context) {
	//ReturnJson(c, http.StatusBadRequest, http.StatusBadRequest, my_errors.ErrorsTokenBaseInfo, "")
	ReturnJson(c, http.StatusOK, consts.JwtTokenInvalid, my_errors.ErrorsTokenBaseInfo, "")
	//终止可能已经被加载的其他回调函数的执行
	c.Abort()
}
...
func ErrorTokenAuthFail(c *gin.Context) {
	...
	ReturnJson(c, http.StatusOK, consts.JwtTokenInvalid, my_errors.ErrorsNoAuthorization, "")
	//终止可能已经被加载的其他回调函数的执行
	c.Abort()
}
...
func ErrorTokenRefreshFail(c *gin.Context) {
	ReturnJson(c, http.StatusOK, http.StatusBadRequest, my_errors.ErrorsRefreshTokenFail, "")
	//终止可能已经被加载的其他回调函数的执行
	c.Abort()
}
...
func ValidatorError(c *gin.Context, err error) {
	if errs, ok := err.(validator.ValidationErrors); ok {
		...
		if strings.ReplaceAll(strings.ToLower(errStr), " ", "") == "multipart:nextpart:eof" {
			ReturnJson(c, http.StatusOK, consts.ValidatorParamsCheckFailCode, consts.ValidatorParamsCheckFailMsg, gin.H{"tips": my_errors.ErrorNotAllParamsIsBlank})
		} else {
			ReturnJson(c, http.StatusOK, consts.ValidatorParamsCheckFailCode, consts.ValidatorParamsCheckFailMsg, gin.H{"tips": errStr})
		}
	}
	c.Abort()
}
