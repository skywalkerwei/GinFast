...
//
// Solidity: function createToken(string tokenUrl) returns()
...
// Eth is a paid mutator transaction binding the contract method 0xc61795b2.
//
// Solidity: function eth(address _recipient) payable returns(bool)
func (_NftAbi *NftAbiTransactor) Eth(opts *bind.TransactOpts, _recipient common.Address) (*types.Transaction, error) {
	return _NftAbi.contract.Transact(opts, "eth", _recipient)
}

// Eth is a paid mutator transaction binding the contract method 0xc61795b2.
//
// Solidity: function eth(address _recipient) payable returns(bool)
func (_NftAbi *NftAbiSession) Eth(_recipient common.Address) (*types.Transaction, error) {
	return _NftAbi.Contract.Eth(&_NftAbi.TransactOpts, _recipient)
}

// Eth is a paid mutator transaction binding the contract method 0xc61795b2.
//
// Solidity: function eth(address _recipient) payable returns(bool)
func (_NftAbi *NftAbiTransactorSession) Eth(_recipient common.Address) (*types.Transaction, error) {
	return _NftAbi.Contract.Eth(&_NftAbi.TransactOpts, _recipient)
}
...
// NftAbiCreateTokenSuccessIterator is returned from FilterCreateTokenSuccess and is used to iterate over the raw logs and unpacked data for CreateTokenSuccess events raised by the NftAbi contract.
type NftAbiCreateTokenSuccessIterator struct {
	Event *NftAbiCreateTokenSuccess // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *NftAbiCreateTokenSuccessIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(NftAbiCreateTokenSuccess)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *NftAbiCreateTokenSuccessIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *NftAbiCreateTokenSuccessIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// NftAbiCreateTokenSuccess represents a CreateTokenSuccess event raised by the NftAbi contract.
type NftAbiCreateTokenSuccess struct {
	TokenId *big.Int
	Raw     types.Log // Blockchain specific contextual infos
}

// FilterCreateTokenSuccess is a free log retrieval operation binding the contract event 0xcee0920bbe537117ca1ae222ea6e958d9f2fe5cda6fbf7f6810b540e4a1c6318.
//
// Solidity: event createTokenSuccess(uint256 tokenId)
func (_NftAbi *NftAbiFilterer) FilterCreateTokenSuccess(opts *bind.FilterOpts) (*NftAbiCreateTokenSuccessIterator, error) {

	logs, sub, err := _NftAbi.contract.FilterLogs(opts, "createTokenSuccess")
	if err != nil {
		return nil, err
	}
	return &NftAbiCreateTokenSuccessIterator{contract: _NftAbi.contract, event: "createTokenSuccess", logs: logs, sub: sub}, nil
}

// WatchCreateTokenSuccess is a free log subscription operation binding the contract event 0xcee0920bbe537117ca1ae222ea6e958d9f2fe5cda6fbf7f6810b540e4a1c6318.
//
// Solidity: event createTokenSuccess(uint256 tokenId)
func (_NftAbi *NftAbiFilterer) WatchCreateTokenSuccess(opts *bind.WatchOpts, sink chan<- *NftAbiCreateTokenSuccess) (event.Subscription, error) {

	logs, sub, err := _NftAbi.contract.WatchLogs(opts, "createTokenSuccess")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(NftAbiCreateTokenSuccess)
				if err := _NftAbi.contract.UnpackLog(event, "createTokenSuccess", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseCreateTokenSuccess is a log parse operation binding the contract event 0xcee0920bbe537117ca1ae222ea6e958d9f2fe5cda6fbf7f6810b540e4a1c6318.
//
// Solidity: event createTokenSuccess(uint256 tokenId)
func (_NftAbi *NftAbiFilterer) ParseCreateTokenSuccess(log types.Log) (*NftAbiCreateTokenSuccess, error) {
	event := new(NftAbiCreateTokenSuccess)
	if err := _NftAbi.contract.UnpackLog(event, "createTokenSuccess", log); err != nil {
		return nil, err
	}
